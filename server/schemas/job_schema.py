from .__init__ import fields, validate, validates, ValidationError, ma, Job
...
class JobSchema(ma.SQLAlchemySchema):
    ...
    user = fields.Nested(
        "UserSchema", only=("id", "email", "name", "bio", "profile_pic_url", "url")
    )
    hires = fields.Nested("HireSchema", only=("id", "job_seeker_id", "user", "url"))
    url = ma.Hyperlinks(
        {
            "self": ma.URLFor("jobbyid", values=dict(id="<id>")),
